...
const naskahRajaKediri = [
    { text: "Selamat datang, pahlawan muda yang pemberani! Aku adalah Raja Kediri.", mood: "happy" },
    ...
    { text: "Di setiap kecamatan, bersiaplah menemukan rahasia materi yang terkandung dalam buku IPAS-mu, mulai dari Bab 1 hingga Bab 8.", mood: "neutral" },
...
// ================= SISTEM REGISTRASI & MASUK GAME =================
const GAS_URL = "https://script.google.com/macros/s/AKfycbxaIlUvbvk6wfSqrRsIRCzRnFvZmP84swecdr47gIlPbQxvhywnLx0-o8tCsFR5mdgN/exec";
let playerAvatarBase64 = ""; // Variabel penampung gambar

// Fungsi Membaca File Foto
window.previewAvatar = function(event) {
    const file = event.target.files[0];
    if (file) {
        const reader = new FileReader();
        reader.onload = function(e) {
            playerAvatarBase64 = e.target.result;
            document.getElementById('avatar-preview').src = playerAvatarBase64;
        }
        reader.readAsDataURL(file);
    }
};

// Tombol Mulai di Layar Utama (Mencegat Pemain yang Belum Daftar)
...
// Tombol Simpan di Kartu Identitas
const btnSaveRegister = document.getElementById('btn-save-register');
if (btnSaveRegister) {
    btnSaveRegister.addEventListener('click', () => {
        const nameInput = document.getElementById('player-name-input').value.trim();
        if (nameInput === "") {
            alert("Isi namamu dulu, Pahlawan Cilik!");
            return;
        }
        
        btnSaveRegister.innerText = "MENYIAPKAN...";
        btnSaveRegister.disabled = true;

        // Simpan Identitas ke Memori HP
        let finalAvatar = playerAvatarBase64 !== "" ? playerAvatarBase64 : document.getElementById('avatar-preview').src;
        localStorage.setItem('playerName', nameInput);
        localStorage.setItem('playerAvatar', finalAvatar);

        // API 1: Kirim Pendaftaran Awal secara diam-diam (Background Fetch) agar anak tidak lama menunggu
        fetch(GAS_URL, {
            method: 'POST',
            body: JSON.stringify({ type: 'register', nama: nameInput, foto: finalAvatar, skor: globalScore })
...
// Mesin Pemutar Intro Maps (Telah Diekstrak)
function masukKeGame() {
    ...
    if (!isRajaDone) {
        setTimeout(() => {
            // Panggil nama pemain secara dinamis dari LocalStorage!
            let welcomeName = localStorage.getItem('playerName') || "pahlawan muda";
            naskahRajaKediri[0].text = `Selamat datang, ${welcomeName} yang pemberani! Aku adalah Raja Kediri.`;
        ...
        () => {
            playTransition(data.bab, data.title, () => {
                ...
                if(districtId === 'kandangan') {
                    ...
                    if (!isNagataDone) {
                        setTimeout(() => {
                            // Mengambil nama pemain dari memori
                            let welcomeName = localStorage.getItem('playerName') || "Cilik";
                            
                            // Mengubah teks sapaan Nagata (Kalimat Pertama dan Terakhir) secara dinamis
                            naskahNagata[0].text = `Salam, Petualang ${welcomeName}! Selamat datang di wilayah misterius Kandangan!`;
                            naskahNagata[5].text = `Kehabisan kristal nyawa akan membawamu ke ruang Hukuman Bayangan! Semoga berhasil, Petualang ${welcomeName}!`;
...
btnPengaturan.addEventListener('click', () => {
    // Render data profil terkini ke layar pengaturan
    const currName = localStorage.getItem('playerName') || "Pahlawan Tanpa Nama";
    const currAvatar = localStorage.getItem('playerAvatar') || "data:image/svg+xml;utf8,<svg viewBox='0 0 16 16' xmlns='http://www.w3.org/2000/svg'><path fill='%23bdc3c7' d='M8 2a3 3 0 100 6 3 3 0 000-6zm-5 9a5 5 0 0110 0v3H3v-3z'/></svg>";
    
    const settingsNameEl = document.getElementById('settings-player-name');
    const settingsAvatarEl = document.getElementById('settings-avatar-preview');
    
    if(settingsNameEl) settingsNameEl.innerText = currName;
    if(settingsAvatarEl) settingsAvatarEl.src = currAvatar;
...
if (btnEditProfile) {
    btnEditProfile.addEventListener('click', () => {
        // Muat data lama ke modal saat dibuka
        document.getElementById('edit-player-name-input').value = localStorage.getItem('playerName') || "";
        document.getElementById('edit-avatar-preview').src = localStorage.getItem('playerAvatar') || "data:image/svg+xml;utf8,<svg viewBox='0 0 16 16' xmlns='http://www.w3.org/2000/svg'><path fill='%23bdc3c7' d='M8 2a3 3 0 100 6 3 3 0 000-6zm-5 9a5 5 0 0110 0v3H3v-3z'/></svg>";
        editAvatarBase64 = ""; // Reset memori foto pending
...
if (btnSaveEditProfile) {
    btnSaveEditProfile.addEventListener('click', () => {
        const nameInput = document.getElementById('edit-player-name-input').value.trim();
        if (nameInput === "") {
            alert("Nama tidak boleh kosong!");
            return;
        }

        btnSaveEditProfile.innerText = "MENYIMPAN...";
        btnSaveEditProfile.disabled = true;

        let finalAvatar = editAvatarBase64 !== "" ? editAvatarBase64 : document.getElementById('edit-avatar-preview').src;
        
        // Update memori HP (LocalStorage)
        localStorage.setItem('playerName', nameInput);
        localStorage.setItem('playerAvatar', finalAvatar);

        // Update tampilan di Layar Pengaturan seketika
        document.getElementById('settings-player-name').innerText = nameInput;
        document.getElementById('settings-avatar-preview').src = finalAvatar;

        // Kirim pembaruan ke Google Sheets diam-diam agar server tahu ada perubahan data / foto baru
        ...
        // Tutup modal
        setTimeout(() => {
            btnSaveEditProfile.innerText = "SIMPAN";
...
// Fungsi Merender Data Papan Skor (Gabungan Asli & Dummy) DARI SERVER ONLINE
function renderDummyScores() {
    // Munculkan teks loading saat mengambil data dari internet
    scoreList.innerHTML = "<li style='color:#f1c40f; text-align:center; font-size: 1.5cqw; padding: 2cqw;'>Menghubungkan ke satelit pusat...<br>Memuat data peringkat...</li>";

    const dummyData = [
        { name: "Pak Aprel (Guru)", score: 9999 },
        { name: "Mbah Darmi", score: 1500 },
        { name: "Pejuang Nilai 100", score: 1200 },
        { name: "Detektif Cilik", score: 950 },
        { name: "Siswa Teladan", score: 750 },
        { name: "Si Paling IPAS", score: 600 },
        { name: "Jagoan Kandangan", score: 450 },
        { name: "Anak Rajin", score: 300 },
        { name: "Murid Baru", score: 150 },
        { name: "Si Suka Jajan", score: 50 }
    ];

    // Fungsi kecil untuk menampilkan skor ke layar setelah data siap
    const tampilkanKeLayar = (onlineScores) => {
        let allScores = [...dummyData, ...onlineScores];
        
        // Pastikan tidak ada nama kembar (ambil skor tertingginya saja)
        let uniqueScores = [];
        let nameMap = {};
        allScores.forEach(item => {
            if (!nameMap[item.name] || item.score > nameMap[item.name]) {
                nameMap[item.name] = item.score;
            }
        });
        for (let name in nameMap) {
            uniqueScores.push({ name: name, score: nameMap[name] });
        }

        // Urutkan dari terbesar ke terkecil, lalu ambil Top 10 Saja
        uniqueScores.sort((a, b) => b.score - a.score);
        uniqueScores = uniqueScores.slice(0, 10);

        scoreList.innerHTML = ""; // Bersihkan teks loading
        
        uniqueScores.forEach((data, index) => {
            const rank = index + 1;
            let rankClass = "";
    ...
    // Proses mengambil data dari Google Apps Script
    fetch(GAS_URL)
    .then(response => response.json())
    .then(data => {
        if (data.status === "success" && data.data) {
            tampilkanKeLayar(data.data); // Gunakan data dari Internet
        } else {
            tampilkanKeLayar([]); // Jika data kosong, tampilkan dummy saja
        }
    })
    .catch(error => {
        console.log("Internet offline. Menggunakan data lokal.", error);
        let savedScores = JSON.parse(localStorage.getItem('leaderboard')) || [];
        tampilkanKeLayar(savedScores); // Pakai data lokal (Fallback) jika tidak ada sinyal
    });
}
            ...
            if(dialogData.isBoss) {
                ...
                if (typeof startJurnal === 'function') {
                    startJurnal(currentMissionId, () => {
                        let missionScore = globalScore - (window.scoreBeforeMission || 0);
                        if (typeof showScoreInfo === 'function') {
                            // TAMBAHAN PENTING: Kirimkan ID Misinya juga (1, 2, 3...)
                            showScoreInfo(missionScore, currentMissionId); 
                        } else {
                            exitMissionScreen();
                        }
                    });
...
function enterMissionScreen(title1, title2, missionId = 1) {
    // REKAM SKOR SAAT INI UNTUK MENGHITUNG POIN MISI NANTI
    window.scoreBeforeMission = globalScore;
